...
export const abortShamirRecovery = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updatedRequestsRes = await db.query(
      `UPDATE shamir_recovery_requests SET status = CASE WHEN expiry_date IS NULL OR expiry_date > NOW() THEN 'ABORTED'::shamir_status ELSE 'CLOSED'::shamir_status END WHERE vault_id=$1 AND status='PENDING' RETURNING id`,
      [vaultId],
    );
    await db.query(
