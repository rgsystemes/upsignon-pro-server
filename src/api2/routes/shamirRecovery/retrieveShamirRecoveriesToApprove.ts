...
export const retrieveShamirRecoveriesToApprove = async (
  ...
  try {
    ...
    const pendingRecoveryRequestsRes = await db.query(
      ...
      INNER JOIN shamir_holders AS sh ON sh.vault_id=ss.holder_vault_id AND sh.shamir_config_id=sss.shamir_config_id
      WHERE
        srr.status='PENDING'
    ...
    res.status(204).json({ pendingRecoveryRequests: pendingRecoveryRequestsRes.rows });
    return;
  } catch (e) {
