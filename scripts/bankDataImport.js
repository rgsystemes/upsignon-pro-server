...
async function importFunction(data, bankId, dbConnection, resellerId = null) {
  ...
  if (data.shamir_recovery_requests) {
    for (var i = 0; i < data.shamir_recovery_requests.length; i++) {
      ...
      await dbConnection.query(
        'INSERT INTO shamir_recovery_requests (vault_id, public_key, protected_private_key, shamir_config_id, created_at, completed_at, status, expiry_date, denied_by) VALUES ($1,$2,$3,$4,$5,$6,$7,$7,$8,$9)',
        [
          srr.newVaultId,
          srr.public_key,
          srr.protected_private_key,
